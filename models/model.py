...
class Model:
    ...
    def studentCount (self, nameTable):
        with connection().cursor() as cursor:

            query = f"SELECT COUNT(*) AS StudentCount FROM {nameTable} WHERE StudyForm = 'Day'"
            cursor.execute(query)
            return cursor.fetchall()
        connection().close()

    def getINFOcurculumn (self, nameTable):
        with connection().cursor() as cursor:
            query = f"SELECT DisciplineName, Hours, ReportingForm FROM {nameTable} WHERE DisciplineName = 'Программирование' "
            cursor.execute(query)
            return cursor.fetchall()
        connection().close()
